...
const App = () => {
  ...
  const fetchCards = async () => {
    const res = await Axios.get("http://192.168.1.138:8080/card/findAll")
    const data = await res.data
    return data
  }
   ...
   const fetchPlayers = async () => {
    const res = await Axios.get("http://192.168.1.138:8080/player/findAll")
    const data = await res.data
    return data
  }
  ...
  const makeGameArr = () => {
    ...
    const getGameIDs = async ( id ) => {
      try {
        ...
        const res = await Axios.get(`http://192.168.1.138:8080/game/findOne/${id}`)
        IDArr = res.data
        gameArr = res.data
      } catch (err) {
        console.log(err)
