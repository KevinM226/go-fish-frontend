...
export const PlayGamePage = ({ setCurrentPage, mongoGameID, gameData, playerData, setPlayerData, setGameData, cardData, mongoPlayerID }) => {
    ...
    const fillHand = async() => {
        for(let i = 0; i < 6; i++){
            try{
                ...
                const updatedPlayer = await Axios.put(`http://192.168.1.138:8080/player/addCard/${mongoGameID}`, { card: holdCard._id })
            } catch (err) {
                console.error(err)
            }
        }
    ...
    const fetchPlayer = async () => {
        const res = await Axios.get(`http://192.168.1.138:8080/player/findOne/${mongoPlayerID}`)
    ...
    const updateTurn = async ( gName, pName ) => {
        try {
            const updatedGame = await Axios.put(`http://192.168.1.138:8080/game/changeTurn/${mongoGameID}`, { playerName: "holdPlayerIDs" })
        } catch (err) {
            console.error(err)
        }
    }
